...
{
    char[,] fence = new char[fenceHeight, userInput.Length]; //TWORZENIE PŁOTU
    ...
    for (int i = 0; i < userInput.Length; i++)  //PIERWSZY MODUŁ ZLICZA ILE RAZY W KAZDYM RZEDZIE PLOTU POJAWI SIE LITERA
    {
        array[row]++;
    ...
    for (row = 0; row < fenceHeight; row++) //DRUGI MODUŁ NA PODSTAWIE PIERWSZEGO DZIELI WYRAZ NA LITERY WYSTEPUJACE
                                            //W DANYM RZEDZIE PLOTU
    {
    ...
    for (int i = 0; i < userInput.Length; i++) //TRZECI MODUL WPISUJE LICZBY ZGODNIE Z ZASADA SZYFROWANIA PLOTOWEGO
                                               //WYBIERAJAC LITERY Z PODZIELONYCH JUZ WCZESNIEJ NA ODPOWIEDNIE RZEDY
    {
    ...
    string decryptedMessage = "";   //CZWARTY MODUL ODCZYTUJE ZGODNIE Z ZASADA SZYFROWANIA PLOTOWEGO LITERY I UKLADA
                                    //JE W DAJACY SIE JUZ ODCZYTAC WYRAZ/ZDANIE
